        ...
        for row in range(rows):
            if col == 0:  # b
                if row < len(b):
                    ...
                    table[row][col] = opportunities[row-len(b)][1]
...
def get_resolutions(table: list[list]) -> tuple[int, int, float]:
    ...
    c_values = table[-1].copy()
    c_index_0 = c_values.index(0)
    c_values.pop(0)
    print(f"c_0 = {c_index_0}")

    resolution_col_value = min(c_values)
    resolution_col = c_values.index(resolution_col_value) + 1

    c_values.insert(0, c_index_0)

    print(f"res_col_v = {resolution_col_value}, \n "
          f"res_col = {resolution_col}")
    ...
    for row in range(rows - 1):
        if table[row][resolution_col] != 0 and (b_col[row] / table[row][resolution_col] >= 0):
